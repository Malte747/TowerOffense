...
public class TowerGridPlacement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        BuildSoundList(); //Liste F�r TowerPlaceSFX
    }
    ...
    public void PlaceTower(int number)
    {
        ...
        if (GridMouseInput.gridBehindTower && placeLocationValid(number,grid.CellToWorld(GridPlacementSystem.gridPosition + towerRotationCorrection) ) && towerStats.goldCost <= gameManager.defenderGold && towerStats.supplyCost + gameManager.defenderSupply <= gameManager.defenderMaxSupply)
        {
            ...
            if(TowerPlaceSound.Count > number) towerSFX.PlayTowerSound(TowerPlaceSound[number]); //Spielt die gew�nschte SFX Nummer
    ...
    private void BuildSoundList()
    {
        ...
        //Platziert eine Listen Platz f�r TowerIndex Nummer mit passendem Sound im Audiomanager.
